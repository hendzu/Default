...
echo '<div id="vorm">';
    ...
    foreach ($conn->query($sql) as $row) {
        echo '<input type="radio" id="p1" class="css-checkbox" name="partei" value='.$row[0].'>'.$row[1].'<br>';
    }
    ...
    foreach ($conn->query($sql) as $row) {
        echo '<input type="radio" id="p2" class="css-checkbox" name="piirkond" value='.$row[0].'>'.$row[1].'<br>';
    }
