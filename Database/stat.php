...
<?php
    $sql ="Select kandidaadid.Nimi, count(*), parteid.Nimi From haaled h join kandidaadid k on 
        h.kandidaat_id=k.id JOIN parteid p ON p.id=k.partei_id group by kandidaadid.id;";
    echo "<table>";
    echo "<tr><th>Kandidaat</th><th>Partei</th><th>Hääli</th></tr>";
    foreach ($conn->query($sql) as $row) {
